...
@processorHoc(
  'pngquant 通过把 png 转换成包含透明通道的8位图片来缩小文件体积（通常来说比24/32位图片要小60%～80%）',
)
class Pngquant extends PureComponent {
  ...
  render() {
    ...
    return (
      <Fragment>
        <ProcessItem label={'floyd'} tooltip={'控制抖动的级别'}>
        ...
        <ProcessItem label={'nofs'} tooltip={'禁用 Floyd-Steinberg 抖动'}>
          {getFieldDecorator('nofs', {
            valuePropName: 'checked',
            initialValue: this.state.nofs,
          })(<Switch size="small" />)}
        </ProcessItem>
        <ProcessItem label={'品质'} extra={'0 到 100 之间，同 JPEG'}>
        ...
        <ProcessItem
          label={'速度'}
          extra={
            '速度/质量的权衡，1 (最佳质量) 到 10 (最快速度) 之间。速度 10 对比默认值 (速度 3)，以 5% 的效果损失换来 8 倍的速度提升。'
          }
        >
