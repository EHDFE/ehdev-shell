...
export default class TerminalComponent extends PureComponent {
  ...
  componentDidMount() {
    this.socket = new WebSocket(
      `ws://${HOST}:8484/${encodeURIComponent(this.props.messageId)}`,
    );
  ...
  fit(terminal, needMeasure) {
    if (this.props.active) {
      setTimeout(() => {
        needMeasure &&
          terminal._core.charMeasure.measure(terminal._core.options);
        terminal.fit();
      }, 100);
    }
  }
  ...
  render() {
    ...
    return (
      <div
        ...
        className={classnames(className, styles.Terminal__Wrapper, {
